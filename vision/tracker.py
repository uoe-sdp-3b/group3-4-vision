...
class RobotTracker(Tracker):
    ...
    def getRobotOrientation(self, frame, side, position):
        ...
        if self.num_pink[position] == 3:
            ...
            orientation_pink = self.getKClosestContours(3, center, pink_contours)
            orientation_pink = self.getKFurthestContours(2, center, orientation_pink)
    ...
    def opponent_defender_coordinates(self, frame):
        ...
        return self.getRobotCoordinates(frame, 'opponent', 'defender')


    def opponent_attacker_coordinates(self, frame):
        
        return self.getRobotCoordinates(frame, 'opponent', 'attacker')


    def our_defender_coordinates(self, frame):
        
        return self.getRobotCoordinates(frame, 'us', 'defender')


    def our_attacker_coordinates(self, frame):
        
        return self.getRobotCoordinates(frame, 'us', 'attacker')   
